...
@Injectable()
export class Cattalogue {
  constructor() // private readonly domainEvents: DomainEvents // private readonly database: CatalogueDatabase,
  {}
  async addBook(author: string, title: string, isbn: string) {
    // const book = Book.create(isbn, author, title);
    // await this.database.saveNewBook(book);
    // return Result.Success;
  }

  async addBookInstance(isbn: string, bookType) {
    // return pipe(
